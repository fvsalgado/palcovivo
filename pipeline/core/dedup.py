...
def merge_sessions(events: list[dict]) -> list[dict]:
    ...
    for event in events:
        ...
        if key in merged_groups:
            ...
            # Usar `or ""` em vez de default="" para cobrir valores None explícitos
            all_dates.sort(key=lambda d: (d.get("date") or "", d.get("time_start") or ""))
            base["dates"] = all_dates
            base["total_sessions"] = len(all_dates)
            base["date_first"] = (all_dates[0].get("date") or None) if all_dates else None
            base["date_last"] = (all_dates[-1].get("date") or None) if all_dates else None
